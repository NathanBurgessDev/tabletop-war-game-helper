...
An RFID based approach is also used by _Steve Hinske_ and _Marc Langheinrich_ in their paper @rfid-based which places an antenna grid below the game board to detect RFID chips in pieces. This allowed them to find what chip is in range of what antenna, allowing them to find the general location of a game piece. This worked particularly well for larger models where you could put RFID chips far away from each-other on the model. Using the known positions of the chips and dimensions of the model combined with which antenna said chips are in range of allows you to determine an accurate position of each model. They also go into alternate RFID approaches which will be discussed later when outlining the chosen methodology.
...
+ Detection of the models and terrain to create a virtual representation of the game board.
    ...
    + The system must be able to identify between different miniatures.
    + The system must aim to be noninvasive to the miniatures.
    + The system must be ambivalent to a model's shape and colour.
    + The system must be able to complete this task whilst being accessible to the average miniature war-gamer.
...
An RFID approach is the somewhat obvious solution. This would involve embedding RFID chips underneath the bases of the miniatures. Then some method of reading these chips would then need to be embedded either within or underneath the game board. There are a number of different approaches that could be taken to locating RFID chips which have been outlined by _Steve Hinske_ and _Marc Langheinrich_ @rfid-based in their work on a similar project.
...
The approach previously mentioned in _Steve Hinske_ and _Marc Langheinrich_'s paper, which utilised an antenna grid below the game board, seemed promising. 
...
The tag would need to be unobstructive to the game (big enough to be seen by the camera but small enough as to not get in the way of the game whilst also not looking too "out of place"). This ruled out sticking QR codes on top of miniatures, or attaching them to wires a pointed upwards from a miniatures base. This meant a unique design was needed.
...
The next step was to find a way to detect the rim. The best solution found for this was _openCV_ @openCV. An alternative image processing suit would have been to utilise _MATLAB_, but _openCV_ can be used directly within _Python_. As a result, the final system could be written entirely within _Python_ signficantly streamlining the development process without needing to connect _MATLAB_ with an external program for the game board logic / display. 
